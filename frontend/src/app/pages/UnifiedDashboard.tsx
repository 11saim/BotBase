...
export function UnifiedDashboard() {
  ...
  const filteredBots = bots.filter(
    (bot) => activeFilter === "all" || bot.status === activeFilter,
  );
    ...
    switch (action) {
      ...
      case "duplicate":
        if (bot) {
          setBots([
            ...bots,
            { ...bot, id: bots.length + 1, name: `${bot.name} (Copy)` },
          ]);
          setToast({ message: `Bot duplicated!`, type: "success" });
        }
        break;
      case "retrain":
        setToast({ message: "Retraining started...", type: "info" });
        break;
      case "copy":
        navigator.clipboard.writeText(
          `<script src="https://botbase.ai/widget.js" data-bot-id="${botId}"></script>`,
        );
      ...
      case "pause":
        ...
        setToast({
          message: `Bot ${bot?.status === "active" ? "paused" : "resumed"}!`,
          type: "success",
        });
        break;
      case "delete":
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <div
        className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-white border-r flex flex-col transition-transform duration-300 lg:translate-x-0 ${
          mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* Close button — mobile only */}
        <button
          className="absolute top-4 right-4 lg:hidden p-1.5 hover:bg-gray-100 rounded-lg"
          onClick={() => setMobileMenuOpen(false)}
        >
          <X size={18} />
        </button>
        ...
        <div className="p-6">
          ...
          >
            ...
            <span className="font-semibold">Bot Base</span>
          </Link>
        </div>
        ...
        <nav className="flex-1 px-4">
          <div className="mb-6">
            ...
            <Link
              ...
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-100 text-black font-medium mb-1"
            >
              <LayoutGrid size={18} />
              Dashboard
            </Link>
          ...
          <div>
            ...
            {[
              ...
              <Link
                ...
                key={item.label}
                to={item.to}
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-600 hover:bg-gray-50 hover:text-black transition-all mb-1"
              >
                <item.icon size={18} />
                {item.label}
              </Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl p-6 border">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex gap-2">
                    {["7d", "30d", "90d"].map((period) => (
                      <button
                        key={period}
                        className="px-3 py-1 text-xs font-medium rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors"
                      >
                        {period}
                      </button>
                    ))}
                ...
                <div className="flex items-end gap-2 h-48">
                  {[45, 70, 55, 85, 65, 80, 95, 70, 90, 65, 80, 92].map(
                    (height, i) => (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredBots.map((bot) => (
                  <div
                    key={bot.id}
                    className="bg-white rounded-2xl p-6 border-2 hover:border-black transition-all relative group"
                    style={{
                      borderColor:
                        bot.status === "active" ? "#10b981" : "#e5e7eb",
                    }}
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <button
                        onClick={() =>
                          setShowMenu(showMenu === bot.id ? null : bot.id)
                        }
                      ...
                      {showMenu === bot.id && (
                        <div
                          ref={menuRef}
                          className="absolute top-14 right-4 sm:right-6 w-48 bg-white rounded-xl border shadow-xl z-20 py-1"
                        >
                          {[
                            ...
                            <button
                              key={item.action}
                              onClick={() =>
                                handleMenuAction(bot.id, item.action)
                              }
                ...
                <Link
                  to="/create-bot"
                  className="bg-white rounded-2xl p-6 border-2 border-dashed hover:border-solid hover:border-black transition-all flex flex-col items-center justify-center min-h-[300px] group"
                >
                  <Plus
                    size={32}
                    className="text-gray-400 group-hover:text-black transition-colors mb-3"
                  />
                  <p className="font-medium text-gray-600 group-hover:text-black transition-colors">
                    Create New Bot
                  </p>
                </Link>
              </div>
                ...
                {[
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (answer)
                        setToast({ message: "Answer saved!", type: "success" });
                    }}
                  >
                    ...
                    <button className="self-start sm:self-auto px-4 py-2 text-sm font-medium border rounded-xl hover:border-black transition-all">
                      Add Answer
                    </button>
                  </div>
                ))}
