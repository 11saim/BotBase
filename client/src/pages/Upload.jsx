  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (tab === 'file') {
        ...
        response = await fetch(`${API}/ingest/file`, { method: 'POST', body: formData });
        ...
        response = await fetch(`${API}/ingest/url`, {
          ...
          body: JSON.stringify({ url: url.trim(), botName: botName || url })
        });
    ...
    <div style={styles.page} className="dot-grid">
      <div style={styles.card}>
        ...
        <div style={styles.header}>
          <div style={styles.logo}>botbase</div>
        ...
        {tab === 'url' && (
          <div style={styles.field}>
            ...
            />
            <div style={styles.hint}>We'll scrape the text content from this page.</div>
          </div>
        )}
        ...
        {tab === 'text' && (
          <div style={styles.field}>
            <textarea
              style={{ ...styles.input, height: 160, resize: 'vertical', padding: '10px 12px' }}
              placeholder="Paste your FAQs, product docs, support content..."
              value={text}
              onChange={e => setText(e.target.value)}
            />
            <div style={styles.hint}>{text.length.toLocaleString()} characters</div>
          </div>
        )}
          ...
          <div style={styles.logBox}>
            {logs.map(log => (
              <div
                ...
                style={{
                  ...styles.logLine,
                  color: log.type === 'error' ? '#cc2222' : log.type === 'success' ? '#1a6b3c' : '#6b6b65'
                }}
              >
