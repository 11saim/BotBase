...
import { postIngestFile, postIngestText } from "../../lib/api";
import {
  defaultAppearance,
  type BotAppearance,
  type BotFontId,
...
export function CreateBotWizardModal({ open, onOpenChange }: Props) {
  ...
  const finish = () => {
    ...
    onOpenChange(false);
    navigate(`/dashboard/bots/${bot.id}`, { replace: true });
  };
      ...
      <DialogContent className="fixed top-1/2 left-1/2 z-50 flex max-h-[min(920px,94vh)] w-[calc(100%-1.25rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 flex-col gap-0 overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-0 shadow-2xl sm:max-h-[92vh]">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 overflow-y-auto lg:grid-cols-[1fr_300px]">
            ...
            <div className="flex flex-wrap gap-3 pb-2">
              {step > 0 && (
                <button
                  type="button"
                  onClick={goBack}
                  className="rounded-xl border border-[var(--border-default)] px-5 py-2.5 text-sm font-medium"
                  style={{ color: "var(--text-primary)" }}
                >
                  Back
                </button>
              ...
              {step < STEPS.length - 1 ? (
                <button
                  ...
                  disabled={!canNext()}
                  onClick={goNext}
                  className="rounded-xl px-6 py-2.5 text-sm font-medium text-white disabled:opacity-40"
                  style={{ background: "var(--text-primary)" }}
                >
                  Continue
                </button>
          ...
          <aside className="border-t border-[var(--border-default)] bg-[var(--bg-secondary)]/80 p-5 lg:border-l lg:border-t-0">
            ...
            <div
              className="mt-4 overflow-hidden rounded-2xl border shadow-[0_8px_30px_rgba(0,0,0,0.08)]"
              style={{
                borderColor: chatShell.border,
